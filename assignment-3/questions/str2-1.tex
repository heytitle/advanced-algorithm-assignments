...
3c \cdot \var{\widetilde{\Phi}(\sigma)}} 
= \pr{|\widetilde{\Phi}(\sigma)- \expt{\widetilde{\Phi}(\sigma)} \geq \
3c \cdot \sqrt(\var{\widetilde{\Phi}(\sigma)}) \cdot \sqrt(\var{\widetilde{\Phi}(\sigma)})} 
\leq \frac{1}{9 \cdot c^2 \cdot 
  \sqrt(\var{\widetilde{\Phi}(\sigma)})} $$
...
The idea of the new algorithm is to run \alg $k$ times, then taking the median of the $k$ return values $\widetilde{\Phi}(\sigma)$ (the Median trick). Before modify the algorithm formally, we make some analysis as follows. \\
...
In conclusion, we modify the algorithm as follow:
